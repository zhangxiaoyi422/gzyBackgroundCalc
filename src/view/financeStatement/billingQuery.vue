<template>
    <div>
                ...
                <ul class="ul clearfix">
                    ...
                    <li class="li clearfix">
                        ...
                        <FormItem class="formItem" prop="keywords">
                            <Input v-model="form.keywords" placeholder="用户名/开票订单号" style="width: 200px" @keyup.native.13&&108="getTableDate(1)"></Input>
                        </FormItem>
                    </li>            
                    <li class="li clearfix">
                        <label>实际开票金额：</label>
        ...
        <div class="fun-tool border-top">
            <span>实际开票金额&nbsp;&nbsp;合计（元）：{{resData.totalMoney}}</span>
            <div class="fr">
            ...
            <div class="exportItem">
                ...
                <CheckboxGroup v-model="exportShowItem_now" @on-change="checkAllGroupChange_export">
                    <template v-for="item in respFieldDesc">
                        <Checkbox :label="item.value" :disabled="item.value == 'endDate'">{{item.label}}</Checkbox>
                    </template>
                </CheckboxGroup>
            </div>
            <div slot="footer">
...
<script>
    ...
    export default {
        computed: {
            basePath(){
                return this.$store.state.basePath + 'invoice/order/exportInvoice.action'
            },
        },
        data() {
            ...
            return {
                ...
                settingShowItem_all:[],
                settingShowItem_default:['endDate','orderNums','userAccount','money','invoiceMoney'],
                settingShowItem_now:['endDate','orderNums','userAccount','money','invoiceMoney'],
                settingShowItem_old:['endDate','orderNums','userAccount','money','invoiceMoney'],
                // 导出
                num:true,
                exportExcelBox:false,
                checkAll_export:false,
                exportShowItem_all:[],
                exportShowItem_default:['endDate','orderNums','userAccount','money','invoiceMoney'],
                exportShowItem_now:['endDate','orderNums','userAccount','money','invoiceMoney'],
                // 排序方式，默认倒序
                order:'desc',
                btn_today:'ghost',
                btn_yesterday:'ghost',
        ...
        methods: {
                ...
                for(let i = 0;i < this.settingShowItem_now.length;i++){
                    ...
                    $.each(this.respFieldDesc, function(key, val) {
                        if(_this.settingShowItem_now[i] == val.value){
                            if(_this.settingShowItem_now[i] == 'endDate'){
                                _this.columns1.push({
                                    title: '开票时间',
                                    key: 'endDate',
                                    sortable : 'custom',
            ...
            handleCheckAll_setting() {
                if (this.checkAll_setting == false) {
                    ...
                    this.settingShowItem_now = ['endDate']
                    this.checkAll_setting = false
                }
            ...
            exportExcelBox_ok(){
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        this.$http.post('/invoice/order/exportInvoice.action', this.qs.stringify({
                            sort:this.order,
                            beginTime: this.form.start,
                            endTime: this.form.end,
                            userAccount:this.form.keywords,
                            beginMoney: this.form.minMoney,
                            endMoney: this.form.maxMoney,
                            userType:this.form.userTypeDefault,
                            registerResource:this.form.userResourceDefault,
                            exportHeaders:exportHeaders
                        })).then(response => {
                            console.log(response)
                            if (response.status == 200) { 
                                this.url =  this.basePath + '?sort=' + this.order + '&beginTime=' + this.form.start 
                                                    + '&endTime=' + this.form.end + '&userAccount=' + this.form.keywords 
                                                    + '&beginMoney=' + this.form.minMoney + '&endMoney=' + this.form.maxMoney 
                                                    + '&userType=' + this.form.userTypeDefault + '&registerResource=' + this.form.userResourceDefault
                                                    + '&exportHeaders=' + exportHeaders
            ...
            handleCheckAll_export() {
                if (this.checkAll_export == false) {
                    ...
                    this.exportShowItem_now = ['endDate']
                    this.checkAll_export = false
                }
                console.log(this.exportShowItem_now)
            },
            ...
            getTableDate(page,order) {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        this.current = page
                        this.$http.get('/invoice/InvoiceReportList.action', {params:{
                            sort:this.order,
                            beginTime: this.form.start,
                            endTime: this.form.end,
                            userAccount:this.form.keywords,
                            ...
                            if (response.data.code == 200) {
                                ...
                                $.each(response.data.data.list, function(key, val) {
                                    switch (val.registerResource) {
                                        case '1':
                                            val.registerResource = '公证云网站'; break
                                        case '2':
                                            val.registerResource = '公证云APP'; break
                                        case '3':
                                            val.registerResource = '第三方网站'; break
                                        case '4':
                                            val.registerResource = '微信城市服务'; break
                                        case '5':
                                            val.registerResource = 'weui版微信'; break
                                        case '6':
                                            val.registerResource = '建行城市服务'; break
                                        case '7':
                                            val.registerResource = 'H5版本在线受理'; break
                                        case '8':
                                            val.registerResource = '支付宝城市服务'; break
                                        case '9':
                                            val.registerResource = '公证录音'; break
                                        case '10':
                                            val.registerResource = '公证签约'; break
                                        case '11':
                                            val.registerResource = '知识产权'; break
                                        case '12':
                                            val.registerResource = '新浪'; break
                                        case '13':
                                            val.registerResource = '旗舰店注册'; break   
                                        case '-':
                                            val.registerResource = '-'; break
                                    }
                                    switch (val.userType) {
                                        case '2':
                                    ...
                                    switch (val.status) {
                                        case '2':
                                            val.status = '开票完成'; break
                                        case '3':
                                            val.status = '已邮寄'; break
                                        case '-':
                                            val.status = '-'; break
                                    }
                                    switch (val.invoiceType) {
                                        case '1':
                                            val.invoiceType = '增值税普通发票'; break
                                        case '2':
                                            val.invoiceType = '增值税专用发票'; break
                                        case '-':
                                            val.invoiceType = '-'; break
                                    }
                                });  
