<template>
    <div>
                ...
                <ul class="ul">
                    ...
                    <li class="li">
                        ...
                        <FormItem class="formItem" prop="keywords">
                            <Input v-model="form.keywords" placeholder="用户名/赠送订单号/赠送说明" style="width: 200px" @keyup.native.13&&108="getTableDate(1)"></Input>
                        </FormItem>
                    </li>            
                    <li class="li">
                        <label>赠送水晶币个数：</label>
                        <FormItem class="formItem" prop="minMoney">
                            <Input v-model="form.minMoney" style="width: 80px" @keyup.native.13&&108="getTableDate(1)"></Input>&nbsp;-&nbsp;
                        </FormItem>
                        <FormItem class="formItem" prop="maxMoney">
                            <Input v-model="form.maxMoney" style="width: 80px" @keyup.native.13&&108="getTableDate(1)"></Input>
                        </FormItem>                        
                    </li>
                    <li class="li">
                        <label>用户类型：</label>
        ...
        <div class="fun-tool border-top">
            <span style="margin-right:30px">赠送水晶币&nbsp;&nbsp;合计（币）：{{resData.accountCrystal}}</span>
            <div class="fr">
            ...
            <div class="exportItem">
                ...
                <CheckboxGroup v-model="exportShowItem_now" @on-change="checkAllGroupChange_export">
                    <template v-for="item in respFieldDesc">
                        <Checkbox :label="item.value" :disabled="item.value == 'reviewedDate'">{{item.label}}</Checkbox>
                    </template>
                </CheckboxGroup>
            </div>
            <div slot="footer">
...
<script>
    ...
    export default {
        computed: {
            basePath(){
                return this.$store.state.basePath + 'crystal/order/exportCrystal.action'
            },
        },
        data() {
            ...
            return {
                ...
                settingShowItem_all:[],
                settingShowItem_default:['reviewedDate','requestOrder','token','Comment'],
                settingShowItem_now:['reviewedDate','requestOrder','token','Comment'],
                settingShowItem_old:['reviewedDate','requestOrder','token','Comment'],
                // 导出
                num:true,
                exportExcelBox:false,
                checkAll_export:false,
                exportShowItem_all:[],
                exportShowItem_default:['reviewedDate','requestOrder','token','Comment'],
                exportShowItem_now:['reviewedDate','requestOrder','token','Comment'],
        ...
        methods: {
                ...
                for(let i = 0;i < this.settingShowItem_now.length;i++){
                    ...
                    $.each(this.respFieldDesc, function(key, val) {
                        if(_this.settingShowItem_now[i] == val.value){
                            if(_this.settingShowItem_now[i] == 'reviewedDate'){
                                _this.columns1.push({
                                    title: '赠送时间',
                                    key: 'reviewedDate',
                                    sortable : 'custom',
            ...
            handleCheckAll_setting() {
                if (this.checkAll_setting == false) {
                    ...
                    this.settingShowItem_now = ['reviewedDate']
                    this.checkAll_setting = false
                }
            ...
            exportExcelBox_ok(){
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        this.$http.post('/crystal/order/exportCrystal.action', this.qs.stringify({
                            sort:this.order,
                            beginTime: this.form.start,
                            endTime: this.form.end,
                            userName:this.form.keywords,
                            beginToken: this.form.minMoney,
                            endToken: this.form.maxMoney,
                            ...
                            if (response.status == 200) { 
                                this.url =  this.basePath + '?sort=' + this.order + '&beginTime=' + this.form.start 
                                                    + '&endTime=' + this.form.end + '&userName=' + this.form.keywords 
                                                    + '&beginToken=' + this.form.minMoney + '&endToken=' + this.form.maxMoney
            ...
            handleCheckAll_export() {
                if (this.checkAll_export == false) {
                    ...
                    this.exportShowItem_now = ['reviewedDate']
                    this.checkAll_export = false
                }
                console.log(this.exportShowItem_now)
            },
            ...
            getTableDate(page,order) {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        this.current = page
                        this.$http.get('/crystal/crystalReportList.action', {params:{
                            sort:this.order,
                            beginTime: this.form.start,
                            endTime: this.form.end,
                            userName:this.form.keywords,
                            ...
                            endToken: this.form.maxMoney,
                            userType:this.form.userTypeDefault,
                            userRegisterResource:this.form.userResourceDefault,
                            pageNo: page,
                            ...
                            if (response.data.code == 200) {
                                ...
                                $.each(response.data.data.list, function(key, val) {
                                    switch (val.userRegisterResource) {
                                        case '1':
                                            val.userRegisterResource = '公证云网站'; break
                                        case '2':
                                            val.userRegisterResource = '公证云APP'; break
                                        case '3':
                                            val.userRegisterResource = '第三方网站'; break
                                        case '4':
                                            val.userRegisterResource = '微信城市服务'; break
                                        case '5':
                                            val.userRegisterResource = 'weui版微信'; break
                                        case '6':
                                            val.userRegisterResource = '建行城市服务'; break
                                        case '7':
                                            val.userRegisterResource = 'H5版本在线受理'; break
                                        case '8':
                                            val.userRegisterResource = '支付宝城市服务'; break
                                        case '9':
                                            val.userRegisterResource = '公证录音'; break
                                        case '10':
                                            val.userRegisterResource = '公证签约'; break
                                        case '11':
                                            val.userRegisterResource = '知识产权'; break
                                        case '12':
                                            val.userRegisterResource = '新浪'; break
                                        case '13':
                                            val.userRegisterResource = '旗舰店注册'; break
                                        case '-':
                                            val.userRegisterResource = '-'; break
                                    }
                                    switch (val.userType) {
                                        case '2':
