<template>
    <div>
        ...
        <div class="fun-tool border-top">
            ...
            <span style="margin-right:20px">充值金额&nbsp;&nbsp;合计（元）：{{resData.rechargeSumMoney}}</span>
            <span>用户退款金额&nbsp;&nbsp;合计（元）：{{resData.refundSumMoney}}</span>
        </div>
...
<script>
    ...
    export default {
        computed: {
            basePath(){
                return this.$store.state.basePath + 'summary/order/exportOrder.action'
            },
        },
        data() {
            return {
        ...
        methods: {
            ...
            exportBilling(dayTime){
                this.$http.post('/summary/order/exportOrder.action', this.qs.stringify({
                    time: dayTime,
                })).then(response => {
                    console.log(response)
                    if (response.status == 200) { 
                        this.url =  this.basePath + '?time=' + dayTime 
            ...
            getTableDate(page,order) {
                ...
                this.current = page
                this.$http.get('/summary/order/queryOrder.action', {params:{
