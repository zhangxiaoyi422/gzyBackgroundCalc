<template>
    <div>
        ...
        <div class="search-area clearfix">
            <Form ref="form" :model="form" :rules="ruleCustom" :label-width="0">
                <ul class="ul">
                    ...
                    <li class="li">
                        <label>关键字搜索：</label>
                        <FormItem class="formItem" prop="keyWord">
                            <Input v-model="form.keyWord" placeholder="用户名/退款订单号" style="width: 200px" @keyup.native.13&&108="getTableDate(1)"></Input>
                        </FormItem>
                    </li>            
                    <li class="li">
                        <label>退款金额：</label>
        ...
        <div class="fun-tool border-top">
            <span>退款金额&nbsp;&nbsp;合计（元）：{{resData.totalMoney}}</span>
            <div class="fr">
            ...
            <div class="exportItem">
                ...
                <CheckboxGroup v-model="exportShowItem_now" @on-change="checkAllGroupChange_export">
                    <template v-for="item in respFieldDesc">
                        <Checkbox :label="item.value" :disabled="item.value == 'endTime'">{{item.label}}</Checkbox>
                    </template>
                </CheckboxGroup>
            </div>
            <div slot="footer">
...
<script>
    ...
    export default {
        computed: {
            basePath(){
                return this.$store.state.basePath + 'fair/ajax/exportRefund.action'
            },
        },
        data() {
            ...
            return {
                ...
                settingShowItem_all:[],
                settingShowItem_default:['endTime','orderNum','userAccount','money'],
                settingShowItem_now:['endTime','orderNum','userAccount','money'],
                settingShowItem_old:['endTime','orderNum','userAccount','money'],
                // 导出
                num:true,
                exportExcelBox:false,
                checkAll_export:false,
                exportShowItem_all:[],
                exportShowItem_default:['endTime','orderNum','userAccount','money'],
                exportShowItem_now:['endTime','orderNum','userAccount','money'],
                // 排序方式，默认倒序
                order:'desc',
                btn_today:'ghost',
                btn_yesterday:'ghost',
        ...
        methods: {
                ...
                for(let i = 0;i < this.settingShowItem_now.length;i++){
                    ...
                    $.each(this.respFieldDesc, function(key, val) {
                        if(_this.settingShowItem_now[i] == val.value){
                            if(_this.settingShowItem_now[i] == 'endTime'){
                                _this.columns1.push({
                                    title: '退款时间',
                                    key: 'endTime',
                                    sortable : 'custom',
            ...
            handleCheckAll_setting() {
                if (this.checkAll_setting == false) {
                    ...
                    this.settingShowItem_now = ['endTime']
                    this.checkAll_setting = false
                }
            ...
            exportExcelBox_ok(){
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        this.$http.post('/fair/ajax/exportRefund.action', this.qs.stringify({
                            sort:this.order,
                            ...
                            if (response.status == 200) { 
                                this.url =  this.basePath + '?sort=' + this.order + '&start=' + this.form.start 
                                                    + '&end=' + this.form.end + '&keyWord=' + this.form.keyWord 
            ...
            handleCheckAll_export() {
                if (this.checkAll_export == false) {
                    ...
                    this.exportShowItem_now = ['endTime']
                    this.checkAll_export = false
                }
                console.log(this.exportShowItem_now)
            },
            ...
            getTableDate(page,order) {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        this.current = page
                        this.$http.post('/fair/ajax/findPageObject.action', this.qs.stringify({
                            sort:this.order,
                            start: this.form.start,
                            end: this.form.end,
                            keyWord:this.form.keyWord,
                            ...
                            if (response.data.code == 200) {
                                ...
                                $.each(response.data.data.value, function(key, val) {
                                    ...
                                    switch (val.status) {
                                        case '2':
                                            val.status = '退款完成'; break
                                        case '3':
                                            val.status = '退款失败'; break
                                        case '-':
                                            val.status = '-'; break
                                    }
                                });  
