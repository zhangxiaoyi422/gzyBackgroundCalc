<template>
    <div>
                ...
                <ul class="ul">
                    ...
                    <li class="li">
                        ...
                        <FormItem class="formItem" prop="keywords">
                            <Input v-model="form.keywords" placeholder="用户名/证据编号/主叫号码/被叫号码" style="width: 200px" @keyup.native.13&&108="getTableDate(1)"></Input>
                        </FormItem>
                    </li>            
                    <li class="li">
                        <label>第三方平台实际计费时长（分）：</label>
        ...
        <div class="fun-tool border-top">
            <span>第三方平台实际计费时长&nbsp;&nbsp;合计（分）：{{resData.totalMoney}}</span>
            <div class="fr">
            ...
            <div class="exportItem">
                ...
                <CheckboxGroup v-model="exportShowItem_now" @on-change="checkAllGroupChange_export">
                    <template v-for="item in respFieldDesc">
                        <Checkbox :label="item.value" :disabled="item.value == 'callInTime'">{{item.label}}</Checkbox>
                    </template>
                </CheckboxGroup>
            </div>
            <div slot="footer">
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                settingShowItem_all:[],
                settingShowItem_default:['callInTime','evidNo','thirdPartySecond','thirdPartyMinute','getThrough','calledPhoneTime','duration'],
                settingShowItem_now:['callInTime','evidNo','thirdPartySecond','thirdPartyMinute','getThrough','calledPhoneTime','duration'],
                settingShowItem_old:['callInTime','evidNo','thirdPartySecond','thirdPartyMinute','getThrough','calledPhoneTime','duration'],
                // 导出
                num:true,
                exportExcelBox:false,
                checkAll_export:false,
                exportShowItem_all:[],
                exportShowItem_default:['callInTime','evidNo','thirdPartySecond','thirdPartyMinute','getThrough','calledPhoneTime','duration'],
                exportShowItem_now:['callInTime','evidNo','thirdPartySecond','thirdPartyMinute','getThrough','calledPhoneTime','duration'],
        ...
        methods: {
                ...
                for(let i = 0;i < this.settingShowItem_now.length;i++){
                    ...
                    $.each(this.respFieldDesc, function(key, val) {
                        if(_this.settingShowItem_now[i] == val.value){
                            if(_this.settingShowItem_now[i] == 'callInTime'){
                                _this.columns1.push({
                                    title: '主叫时间',
                                    key: 'callInTime',
                                    sortable : 'custom',
                                    ...
                                    on: {
                                        click: () => {
                                            this.$router.push({
                                                ...
                                                params: {
                                                    callInTime: params.row.callInTime,
                                                    evidNo: params.row.evidNo,
                                                    resource: params.row.resource,
                                                    userType: params.row.userType,
                                                    account: params.row.account,
                                                    parentAccount: params.row.parentAccount,
                                                    evidType: params.row.evidType,
                                                    comingPhone: params.row.comingPhone,
                                                    calledPhone: params.row.calledPhone,
                                                    calledPhoneTime: params.row.calledPhoneTime,
                                                    thirdPartyMinute: params.row.thirdPartyMinute,
                                                    thirdPartySecond: params.row.thirdPartySecond,
                                                    getThrough: params.row.getThrough,
            ...
            handleCheckAll_setting() {
                if (this.checkAll_setting == false) {
                    ...
                    this.settingShowItem_now = ['callInTime']
                    this.checkAll_setting = false
                }
            ...
            handleCheckAll_export() {
                if (this.checkAll_export == false) {
                    ...
                    this.exportShowItem_now = ['callInTime']
                    this.checkAll_export = false
                }
                console.log(this.exportShowItem_now)
            },
            ...
            getTableDate(page,order) {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        this.current = page
                        this.$http.post('/tel/ajax/findAllRecords.action', this.qs.stringify({
                            sort:this.order,
                            queryStart: this.form.start,
                            queryEnd: this.form.end,
                            keyword:this.form.keywords,
                            thirdPartyStart: this.form.minMoneyToken,
                            thirdPartyEnd: this.form.maxMoneyToken,
                            feeStart: this.form.minMoneyCry,
                            feeEnd: this.form.maxMoneyCry,
                            userType:this.form.userTypeDefault,
                            resource:this.form.userResourceDefault,
                            pageNo: page,
                            ...
                            if (response.data.code == 200) {
                                ...
                                $.each(response.data.data.list, function(key, val) {
                                    switch (val.resource) {
                                        case '1':
                                            val.resource = '公证云网站'; break
                                        case '2':
                                            val.resource = '公证云APP'; break
                                        case '3':
                                            val.resource = '第三方网站'; break
                                        case '4':
                                            val.resource = '微信城市服务'; break
                                        case '5':
                                            val.resource = 'weui版微信'; break
                                        case '6':
                                            val.resource = '建行城市服务'; break
                                        case '7':
                                            val.resource = 'H5版本在线受理'; break
                                        case '8':
                                            val.resource = '支付宝城市服务'; break
                                        case '9':
                                            val.resource = '公证录音'; break
                                        case '10':
                                            val.resource = '公证签约'; break
                                        case '11':
                                            val.resource = '知识产权'; break
                                        case '12':
                                            val.resource = '新浪'; break
                                        case '13':
                                            val.resource = '旗舰店注册'; break
                                        case '-':
                                            val.resource = '-'; break
                                    }
                                    switch (val.userType) {
                                    ...
                                    switch (val.getThrough) {
                                        case 0:
                                            val.getThrough = '未接通'; break
                                        case 1:
                                            val.getThrough = '已接通'; break
                                        case '-':
                                            val.getThrough = '-'; break
                                    }
                                });  
